...
// Sample data for editing - replace with actual data from database
$post_id = isset($_GET['id']) ? $_GET['id'] : 1;
$post = [
    'id' => $post_id,
    'title' => 'Thông báo tuyển sinh năm học 2024-2025',
    'slug' => 'thong-bao-tuyen-sinh-nam-hoc-2024-2025',
    'excerpt' => 'Trường THPT An Lạc thông báo về việc tuyển sinh vào năm học mới với nhiều chính sách ưu đãi cho học sinh.',
    'content' => '<p>Nội dung chi tiết về thông báo tuyển sinh năm học 2024-2025...</p>',
    'featured_image' => 'https://via.placeholder.com/300x200',
    'post_type' => 'announcement', // Default to announcement for this example
    'category_id' => 4, // Thông báo chung (matches announcement type)
    'status' => 'published',
    'tags' => ['thông báo', 'tuyển sinh', 'học sinh'], // Convert to array for easier processing
    'meta_title' => 'Thông báo tuyển sinh THPT An Lạc 2024-2025',
    'meta_description' => 'Thông tin chi tiết về tuyển sinh vào trường THPT An Lạc năm học 2024-2025',
    'allow_comments' => true,
    'is_featured' => false,
    'is_sticky' => false,
    'publish_date' => '2024-08-20T14:30',
            ...
            <div class="modern-card">
                ...
                <div class="modern-card-body">
                    <div class="post-type-grid">
                        <div class="post-type-card <?php echo $post['post_type'] == 'educational_activity' ? 'selected' : ''; ?>"
                            onclick="selectPostType('educational_activity')">
                            <input type="radio" name="post_type" value="educational_activity" id="postTypeEducation"
                                <?php echo $post['post_type'] == 'educational_activity' ? 'checked' : ''; ?>>
                        ...
                        <div class="post-type-card <?php echo $post['post_type'] == 'announcement' ? 'selected' : ''; ?>"
                            onclick="selectPostType('announcement')">
                            <input type="radio" name="post_type" value="announcement" id="postTypeAnnouncement"
                                <?php echo $post['post_type'] == 'announcement' ? 'checked' : ''; ?>>
                ...
                <div class="modern-card-body">
                    <div class="modern-form-group">
                        ...
                        <small class="text-muted">
                            <span id="titleCounter"><?php echo 255 - strlen($post['title']); ?></span> ký tự còn lại
                        </small>
                    </div>
                    ...
                    <div class="modern-form-group">
                        ...
                        <small class="text-muted">
                            <span id="excerptCounter"><?php echo 300 - strlen($post['excerpt']); ?></span> ký tự còn lại
                        </small>
                    </div>
                            ...
                            <div class="slug-preview mb-2">
                                <small class="text-muted">
                                    ...
                                    URL: <span class="fw-bold">../</span><span id="slugPreview"
                                        class="text-primary"><?php echo $post['slug']; ?></span>
                                </small>
                            </div>
                ...
                <div class="modern-card-body">
                    <div class="modern-form-group">
                        ...
                        <textarea class="modern-textarea" name="content" id="content" rows="25"
                            ...
                            style="min-height: 500px; resize: vertical;"><?php echo $post['content']; ?></textarea>
            ...
            <div class="modern-card">
                ...
                <div class="modern-card-body">
                    ...
                    <div class="image-preview mt-3" id="imagePreview"
                        <?php echo $post['featured_image'] ? '' : 'style="display: none;"'; ?>>
                        <?php if ($post['featured_image']): ?>
                        <img id="previewImg" src="src/assets/images/about/thpt-an-lac.png" alt="Preview"
        ...
        <div class="col-lg-4">
            <div class="sidebar-sticky">
                ...
                <div class="modern-card">
                    ...
                    <div class="modern-card-body">
                        <div class="status-options">
                            <div class="status-option <?php echo ($post['status'] === 'draft') ? 'selected' : ''; ?>"
                            ...
                            <div class="status-option <?php echo ($post['status'] === 'published') ? 'selected' : ''; ?>"
                            ...
                            <div class="status-option <?php echo ($post['status'] === 'scheduled') ? 'selected' : ''; ?>"
                                onclick="selectStatus('scheduled')">
                                <input type="radio" name="status" value="scheduled" id="statusScheduled"
                                    <?php echo ($post['status'] === 'scheduled') ? 'checked' : ''; ?>>
                        ...
                        <div class="modern-form-group mt-3" id="scheduleDate"
                            style="display: <?php echo ($post['status'] === 'scheduled') ? 'block' : 'none'; ?>;">
                            ...
                            <input type="datetime-local" class="modern-input" id="publishDate" name="publish_date"
                                value="<?php echo isset($post['publish_date']) ? date('Y-m-d\TH:i', strtotime($post['publish_date'])) : date('Y-m-d\TH:i'); ?>">
                        </div>
                    </div>
                </div>
                ...
                <div class="modern-card">
                    ...
                    <div class="modern-card-body">
                        <div class="status-options">
                            <div class="status-option <?php echo ($post['is_featured']) ? 'selected' : ''; ?>"
                                onclick="toggleSetting('featured')">
                            ...
                            <div class="status-option <?php echo ($post['is_sticky']) ? 'selected' : ''; ?>"
                                onclick="toggleSetting('sticky')">
                                <input type="checkbox" id="isSticky" name="is_sticky"
                                    <?php echo ($post['is_sticky']) ? 'checked' : ''; ?>>
                                <div class="status-icon" style="background: var(--warning-gradient);">
...
let editor;
let selectedTags = <?php echo json_encode($post['tags']); ?>;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Set existing status
    selectStatus('<?php echo $post['status']; ?>');

    // Load categories for current post type
    loadCategoriesForType('<?php echo $post['post_type']; ?>');

    // Load tags for current post type
    loadTagsForType('<?php echo $post['post_type']; ?>');
});
...
function loadCategoriesForType(type) {
    const currentCategoryId = <?php echo $post['category_id']; ?>; // Get current category ID
...
function loadTagsForType(type) {
    ...
    // Handle tag selection based on context
    const currentPostType = '<?php echo $post['post_type']; ?>';

    // Clear selected tags only when switching to different post type (not on initial load or same type)
    if (window.initialLoad !== true && type !== currentPostType) {
        selectedTags = [];
        const tagsList = document.getElementById('tagsList');
        tagsList.innerHTML = '';
    } else if (window.initialLoad === true) {
        // Display existing tags on initial load
        displayExistingTags();
        window.initialLoad = false;
    }

    // Remove selected class from all tag options first
    document.querySelectorAll('.tag-option').forEach(tag => {
        tag.classList.remove('selected');
    });

    // Then mark selected tags if we have any
...
// Display existing tags for edit mode
function displayExistingTags() {
    selectedTags.forEach(tag => {
        // Add tag to selected display
        addTagToDisplay(tag);

        // Mark tag as selected in available tags if it exists
        const tagOption = document.querySelector(`.tag-option[data-tag="${tag}"]`);
        if (tagOption) {
            tagOption.classList.add('selected');
        }
